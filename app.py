...
#         # OCR (optional)
#         src_fixed = input_pdf_path if skip_ocr else ocr_fix_pdf(input_pdf_path, lang=lang, dpi=dpi, optimize=optimize)
...
#         output_pdf_path = str(out_dir / out_name)
...
#             overlay_align={0:0,1:1,2:2,3:3}[overlay_align],  # support 0..3
...
#     # ---- Downloads ----
#     if mode == "all":
#         # zip name your pipeline writes
#         zip_path = out_dir / f"result_{timestamp}.all_all_methods.zip"
#         if zip_path.exists():
#             st.download_button(
#                 "⬇️ Download ALL methods (ZIP)",
#                 data=zip_path.read_bytes(),
#                 file_name=zip_path.name,
#                 mime="application/zip"
#             )
#         else:
#             st.warning(f"ZIP not found at {zip_path}")

#         # also offer individual PDFs
#         made_pdfs = sorted(out_dir.glob(f"result_{timestamp}.all.*.pdf"))
#         if made_pdfs:
#             st.write("Individual method PDFs:")
#             for p in made_pdfs:
#                 st.download_button(
#                     f"⬇️ {p.name}",
#                     data=p.read_bytes(),
...
#                     key=str(p)  # unique key per button
#                 )
#         else:
#             st.warning("No per-method PDFs found.")
#     else:
#         # single-mode path
...
#                     data=f.read(),
#                     file_name=Path(output_pdf_path).name,
#                     mime="application/pdf"
#                 )
#         else:
#             st.error(f"Expected output not found at {output_pdf_path}")
#             # Optional: show what IS in the folder to help debug
#             st.write("Available files:", [p.name for p in out_dir.glob("*")])
